...
**主题**：全库代码审查后的集中修复——数据正确性、口径统一、死代码清零。

- [x] 高危修复：知识图谱引擎侧集成静默失效（路径语义错位）；`kg-recommend` 命令拼写崩溃
- [x] 数据正确性：项目文档交付日期恒错（引用不存在字段）；DEBT 类型目录推导错误；`--force` 丢失 timeline/docs-map
- [x] 口径统一：类型/前缀/状态词表 8 处复制收敛到 schema；导出侧状态表从 schema 派生
- [x] 体验修复：`--offline` 不再渲染必失败的依赖图；plan-sync 不再抹除人工勾选；饼图 100% 消失；正则误提取
- [x] 死代码清零：similarity.js / skills-health.js / git.js 等约 900 行零引用代码；`--filter` 假参数
- [x] 性能与健壮性：仪表板/hook 重复扫描合并；metrics 容错；CSV 转义；错误指引指向真实入口

**验收**：全库审查清单清零；345 用例全绿；CLI 端到端冒烟通过。
