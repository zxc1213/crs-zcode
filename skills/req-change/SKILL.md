...
**第一层：需求内详细记录** — 写入需求目录下的 `CHANGELOG.md`（没有则创建），每个变更一个小节：

```markdown
# 变更日志

## [2026-09-09 14:30] [中] 增加验证码功能
...
小节标题必须带 `[小]`/`[中]`/`[大]` 级别标记（metrics 的变更统计依赖此标记）。

**第二层：项目级历史入账** — 调用引擎记录到项目时间线（`project/timeline.yaml`），已聚合进项目文档的需求会自动替换更新旧区块：

```bash
node "$ZCODE_PLUGIN_ROOT/scripts/requirement-manager/index.js" change --id <需求ID> --level <small|medium|large> --reason "<一句话原因>"
```

引擎返回 `✓ 变更已记录` 即入账成功。注意：

- spec.md 根索引的"变更历史表"只记录**文档自身的修订**（格式调整、章节增删），需求执行变更不写那里
- 不手工编辑 `project/timeline.yaml` 或 `project/changelog.md`（引擎维护）
...
- **绝不**忽略变更对已完成任务的影响
- **绝不**在未确认的情况下删除已完成的工作
- **绝不**跳过变更记录（CHANGELOG.md 与引擎入账两层都要）
- **绝不**让变更"悄悄"发生而不通知用户
- **绝不**手改 project/timeline.yaml（只能经引擎 `change`/`event` 命令写入）
...
- 需求文档: [已更新/无需更新]
- 执行计划: [已更新/无需更新]
- 变更日志: [已记录 CHANGELOG.md]
- 项目级历史: [已引擎入账 timeline]
